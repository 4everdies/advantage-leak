...
ITickable {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, ItemStack stack, EnumFacing side) {
        if (!(inventoryIn instanceof ISidedInventory)) ** GOTO lbl-1000
        isidedinventory = (ISidedInventory)inventoryIn;
        if (side != null) {
            aint = isidedinventory.getSlotsForFace(side);
            for (k = 0; k < aint.length && stack != null && stack.stackSize > 0; ++k) {
        ...
        {
            i = inventoryIn.getSizeInventory();
            for (j = 0; j < i && stack != null && stack.stackSize > 0; ++j) {

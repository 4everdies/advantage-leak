...
extends Item {
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        if ((list = Items.potionitem.getEffects(stack)) != null && !list.isEmpty()) {
            Object s2 = list.get(0).getEffectName();
            s2 = (String)s2 + ".postfix";
            return (String)s + StatCollector.translateToLocal((String)s2).trim();
        }
    ...
    public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
        if (stack.getMetadata() != 0) {
            ...
            if (list != null && !list.isEmpty()) {
                for (PotionEffect potionEffect : list) {
                    Object s1 = StatCollector.translateToLocal(potionEffect.getEffectName()).trim();
                    ...
                    if (potionEffect.getAmplifier() > 0) {
                        s1 = (String)s1 + " " + StatCollector.translateToLocal("potion.potency." + potionEffect.getAmplifier()).trim();
                    }
                    if (potionEffect.getDuration() > 20) {
                        s1 = (String)s1 + " (" + Potion.getDurationString(potionEffect) + ")";
                    }
                    if (potion.isBadEffect()) {
                        tooltip.add(String.valueOf((Object)EnumChatFormatting.RED) + (String)s1);
                        continue;
                    }
                    tooltip.add(String.valueOf((Object)EnumChatFormatting.GRAY) + (String)s1);
                }
            } else {
                String s = StatCollector.translateToLocal("potion.empty").trim();
                tooltip.add(String.valueOf((Object)EnumChatFormatting.GRAY) + s);
            }
            if (!multimap.isEmpty()) {
                tooltip.add("");
                tooltip.add(String.valueOf((Object)EnumChatFormatting.DARK_PURPLE) + StatCollector.translateToLocal("potion.effects.whenDrank"));
                for (Map.Entry entry : multimap.entries()) {
                    ...
                    if (d0 > 0.0) {
                        tooltip.add(String.valueOf((Object)EnumChatFormatting.BLUE) + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())));
                        continue;
                    }
                    if (!(d0 < 0.0)) continue;
                    tooltip.add(String.valueOf((Object)EnumChatFormatting.RED) + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), ItemStack.DECIMALFORMAT.format(d1 *= -1.0), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())));
                }
            }
        }
    }

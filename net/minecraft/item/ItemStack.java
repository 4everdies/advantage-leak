    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
        ...
        if (!(multimap = this.getAttributeModifiers()).isEmpty() && (i1 & 2) == 0) {
            ...
            for (Map.Entry<String, AttributeModifier> entry : multimap.entries()) {
                ...
                if (d0 > 0.0) {
                    list.add(String.valueOf((Object)EnumChatFormatting.BLUE) + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier.getOperation(), DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + entry.getKey())));
                    continue;
                }
                if (!(d0 < 0.0)) continue;
                list.add(String.valueOf((Object)EnumChatFormatting.RED) + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier.getOperation(), DECIMALFORMAT.format(d1 *= -1.0), StatCollector.translateToLocal("attribute.name." + entry.getKey())));
            }
        }

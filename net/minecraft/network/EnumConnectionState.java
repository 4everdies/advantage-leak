...
public enum EnumConnectionState {
    ...
    protected EnumConnectionState registerPacket(EnumPacketDirection direction, Class<? extends Packet> packetClass) {
        ...
        if (bimap.containsValue(packetClass)) {
            String s = String.valueOf((Object)direction) + " packet " + String.valueOf(packetClass) + " is already known to ID " + String.valueOf(bimap.inverse().get(packetClass));
            ...
            for (EnumPacketDirection enumpacketdirection : enumconnectionstate.directionMaps.keySet()) {
                for (Class<? extends Packet> oclass : enumconnectionstate.directionMaps.get((Object)enumpacketdirection).values()) {
                    if (STATES_BY_CLASS.containsKey(oclass) && STATES_BY_CLASS.get(oclass) != enumconnectionstate) {
                        throw new Error("Packet " + String.valueOf(oclass) + " is already assigned to protocol " + String.valueOf((Object)STATES_BY_CLASS.get(oclass)) + " - can't reassign to " + String.valueOf((Object)enumconnectionstate));
                    }
                    try {
                        oclass.newInstance();
                    }
                    catch (Throwable var10) {
                        throw new Error("Packet " + String.valueOf(oclass) + " fails instantiation checks! " + String.valueOf(oclass));
                    }

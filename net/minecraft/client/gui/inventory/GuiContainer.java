...
extends GuiScreen {
    ...
    private Set<Integer> clickedSlots = new HashSet<Integer>();
    private Map<Integer, Long> clickTimes = new HashMap<Integer, Long>();
    ...
    private void renderSlotHighlight() {
        ...
        Iterator<Integer> iterator2 = this.clickedSlots.iterator();
        block0: while (iterator2.hasNext()) {
            Integer slotNumber = iterator2.next();
            Long clickTime = this.clickTimes.get(slotNumber);
            if (clickTime == null) continue;
            long timeSinceClick = currentTime - clickTime;
            float alpha = 1.0f - (float)timeSinceClick / 800.0f;
            if (alpha > 0.0f) {
                for (Slot slot : this.inventorySlots.inventorySlots) {
                    if (slot.slotNumber != slotNumber) continue;
                    int x = slot.xDisplayPosition;
                    int y = slot.yDisplayPosition;
                    int overlayAlpha = (int)(alpha * 150.0f);
                    int borderAlpha = (int)(alpha * 200.0f);
                    int overlayColor = overlayAlpha << 24 | 0xFFFFFF;
                    int borderColor = borderAlpha << 24 | 0xFFFFFF;
                    GuiContainer.drawRect(x, y, x + 16, y + 16, overlayColor);
                    GuiContainer.drawRect(x, y, x + 16, y + 1, borderColor);
                    GuiContainer.drawRect(x, y + 15, x + 16, y + 16, borderColor);
                    GuiContainer.drawRect(x, y, x + 1, y + 16, borderColor);
                    GuiContainer.drawRect(x + 15, y, x + 16, y + 16, borderColor);
                    continue block0;
                }
                continue;
            }
            iterator2.remove();
            this.clickTimes.remove(slotNumber);
        }
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (slot != null) {
            this.clickedSlots.add(slot.slotNumber);
            this.clickTimes.put(slot.slotNumber, Minecraft.getSystemTime());
        }

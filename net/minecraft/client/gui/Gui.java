...
public class Gui {
    ...
    public static void drawModalRectWithCustomSizedTexture(float x, float y, float u, float v, float wH, float wH2, float textureWidth, float textureHeight) {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos(x, y + wH2, 0.0).tex(u * f, (v + wH2) * f1).endVertex();
        worldrenderer.pos(x + wH, y + wH2, 0.0).tex((u + wH) * f, (v + wH2) * f1).endVertex();
        worldrenderer.pos(x + wH, y, 0.0).tex((u + wH) * f, v * f1).endVertex();
        worldrenderer.pos(x, y, 0.0).tex(u * f, v * f1).endVertex();
        tessellator.draw();
    }

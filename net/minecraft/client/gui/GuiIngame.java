...
extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Advantage.INSTANCE.getModuleManager().getModule(PostProcessingModule.class).renderShaders();
        Advantage.INSTANCE.getEventBus().post(new Render2DEvent(partialTicks));
        ...
        if (scoreobjective1 != null) {
            if (Advantage.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
                this.renderAdvantageScoreboard(scoreobjective1, scaledresolution);
            } else {
    ...
    public void renderAdvantageScoreboard(ScoreObjective scoreObjective, ScaledResolution resolution) {
        ...
        boolean useMcFont = FontUtils.getCurrentFont() == FontUtils.getFont("mc") || ScoreboardModule.mcFont.getValue() != false;
        ArrayList<Score> filteredScores = Lists.newArrayList(Iterables.filter(scores, new Predicate<Score>(){
        ...
        ArrayList<Score> displayedScores = filteredScores.size() > 15 ? Lists.newArrayList(Iterables.skip(filteredScores, filteredScores.size() - 15)) : filteredScores;
        ...
        for (Score score : displayedScores) {
            ...
            String scoreText = ScorePlayerTeam.formatPlayerName(playerTeam, score.getPlayerName()) + " " + String.valueOf((Object)EnumChatFormatting.RED);
        ...
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.Vanilla && Advantage.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            startX = resolution.getScaledWidth() - maxWidth;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.Left && Advantage.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            if (startX == 0 || resolution.getScaledWidth() == 0 || maxWidth == 0) {
                return;
            }
            startX = resolution.getScaledWidth() / maxWidth - 14;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.LeftOffset && Advantage.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            if (startX == 0 || resolution.getScaledWidth() == 0 || maxWidth == 0) {
                return;
            }
            startX = resolution.getScaledWidth() / maxWidth;
        }
        if (ScoreboardModule.scoreboardStyle.getValue() == ScoreboardModule.Mode.VanillaOffset && Advantage.INSTANCE.getModuleManager().getModule(ScoreboardModule.class).isEnabled()) {
            startX = resolution.getScaledWidth() - maxWidth - 14;
        }
        float x = startX;
        int rowIndex = 0;
        ...
        int textColor = -1;
        for (Score score : displayedScores) {
            int scorePointsWidth;
            ++rowIndex;
            ScorePlayerTeam playerTeam = scoreboard.getPlayersTeam(score.getPlayerName());
            Object playerName = ScorePlayerTeam.formatPlayerName(playerTeam, score.getPlayerName());
            String scorePoints = String.valueOf((Object)EnumChatFormatting.RED);
            if (score.getScorePoints() == 0) {
                playerName = String.valueOf((Object)EnumChatFormatting.YELLOW) + "Advantage Client";
            }
            int yPos = startY - rowIndex * fontHeight;
            GuiIngame.drawRect(startX - 2, yPos, startX + maxWidth + 2, yPos + fontHeight, backgroundColor);
            int playerNameWidth = useMcFont ? this.mc.fontRendererObj.getStringWidth((String)playerName) : customFr.getStringWidth((String)playerName);
            int n = scorePointsWidth = useMcFont ? this.mc.fontRendererObj.getStringWidth(scorePoints) : customFr.getStringWidth(scorePoints);
            if (useMcFont) {
                this.mc.fontRendererObj.drawString((String)playerName, x + (float)((maxWidth - playerNameWidth - scorePointsWidth) / 2), yPos, textColor, false);
                this.mc.fontRendererObj.drawString(scorePoints, x + (float)maxWidth - (float)scorePointsWidth, yPos, textColor, false);
            } else {
                customFr.drawString((String)playerName, x + (float)((maxWidth - playerNameWidth - scorePointsWidth) / 2), yPos, textColor, false);
                customFr.drawString(scorePoints, x + (float)maxWidth - (float)scorePointsWidth, yPos, textColor, false);
            }
            if (rowIndex != displayedScores.size()) continue;
            String header = scoreObjective.getDisplayName();
            GuiIngame.drawRect(startX - 2, yPos - fontHeight - 1, startX + maxWidth + 2, yPos - 1, backgroundColor);
            GuiIngame.drawRect(startX - 2, yPos - 1, startX + maxWidth + 2, yPos, backgroundColor);
            if (useMcFont) {
                this.mc.fontRendererObj.drawString(header, x + (float)((maxWidth - this.mc.fontRendererObj.getStringWidth(header)) / 2), yPos - fontHeight, textColor, false);
                continue;
            }
            customFr.drawString(header, x + (float)((maxWidth - customFr.getStringWidth(header)) / 2), yPos - fontHeight, textColor, false);
        }
    }
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            Entity entity2 = entityplayer.ridingEntity;
            if (entity2 == null) {
            ...
            } else if (entity2 instanceof EntityLivingBase) {

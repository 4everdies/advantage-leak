...
public class ItemRenderer {
    ...
    public void swing() {
        if (this.mc.thePlayer.getItemInUseCount() > 0 && this.mc.objectMouseOver != null) {
            this.mc.thePlayer.swingItemClient();
        }
    }
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            float var16 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float)Math.PI);
            float swingProgress = abstractclientplayer.swingProgress;
            float convertedProgress = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI * 2.0f);
            this.rotateArroundXAndY(f2, f3);
            this.setLightMapFromPlayer(abstractclientplayer);
            this.rotateWithPlayerRotations(abstractclientplayer, partialTicks);
            GlStateManager.enableRescaleNormal();
            GlStateManager.translate(((Double)CameraModule.x.getValue()).floatValue(), ((Double)CameraModule.y.getValue()).floatValue(), ((Double)CameraModule.z.getValue()).floatValue());
            GlStateManager.pushMatrix();
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    if (Advantage.INSTANCE.getModuleManager().getModule(KillAuraModule.class).isEnabled() && KillAuraModule.target != null && InventoryUtils.isHoldingSword() && KillAuraModule.autoBlocking) {
                        enumaction = EnumAction.BLOCK;
                    }
                    block0 : switch (enumaction) {
                        case NONE: {
                            this.transformFirstPersonItem(f, 0.0f);
                            GlStateManager.scale((Double)CameraModule.scale.getValue(), (Double)CameraModule.scale.getValue(), (Double)CameraModule.scale.getValue());
                            break;
                        }
                        case EAT: 
                        case DRINK: {
                            if (CameraModule.swingEating.getValue().booleanValue() && Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled()) {
                                this.swing();
                            }
                        ...
                        case BLOCK: {
                            if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.dontResetBlock.getValue().booleanValue()) {
                                f = 0.0f;
                            }
                            if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled()) {
                                switch ((CameraModule.AnimationMode)((Object)CameraModule.mode.getValue())) {
                                    case Slide: {
                                        GlStateManager.translate(0.0, -0.08, 0.0);
                                        GlStateManager.translate(0.41f, -0.25f, -0.5555557f);
                                        GlStateManager.translate(0.0f, 0.0f, 0.0f);
                                        GlStateManager.rotate(56.0f, 0.0f, 1.5f, 0.0f);
                                    ...
                                    case Noov: {
                                        ...
                                        GlStateManager.translate(-0.05f, 0.3f, 0.3f);
                                        GlStateManager.rotate(-var16 * 140.0f, 8.0f, 0.0f, 8.0f);
                                        GlStateManager.rotate((float)((double)var16 * Math.PI * 2.0), 8.0f, 0.0f, 8.0f);
                                        GlStateManager.scale((Double)CameraModule.scale.getValue(), (Double)CameraModule.scale.getValue(), (Double)CameraModule.scale.getValue());
                                        break block0;
                                    }
                                    case Spin: {
                    ...
                    if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.fluxSwing.getValue().booleanValue()) {
                        this.transformFirstPersonItem(f, f1);
                    } else {
    ...
    public void renderOverlays(float partialTicks) {
        ...
        if (!this.mc.thePlayer.isSpectator()) {
            ...
            if (this.mc.thePlayer.isBurning()) {
                if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.noFireOverlay.getValue().booleanValue()) {
                    return;
                }

...
implements IResourceManagerReloadListener {
    ...
    public void updateRenderer() {
        ...
        this.viaCollision$previousHeight = this.viaCollision$height;
        this.animatedEyeHeight = this.viaCollision$height = eyeHeight < this.viaCollision$height ? eyeHeight : this.viaCollision$height + (eyeHeight - this.viaCollision$height) * 0.5f;
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            Advantage.INSTANCE.getEventBus().post(mouseOverEvent);
            ...
            for (int j = 0; j < list.size(); ++j) {
                ...
                float f1 = entity1.getCollisionBorderSize() + (entity instanceof EntityPlayer && !entity.isInvisible() ? expand : 0.0f);
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.noHurtCamera.getValue().booleanValue()) {
            return;
        }
        Entity entity = this.mc.getRenderViewEntity();
    ...
    private void orientCamera(float partialTicks) {
        float f;
        Entity entity = this.mc.getRenderViewEntity();
        float f2 = f = this.mc.getRenderViewEntity() instanceof EntityPlayerSP ? ViaMCPFixes.lerp(this.viaCollision$previousHeight, this.viaCollision$height, partialTicks) : entity.getEyeHeight();
        if (entity == this.mc.thePlayer && Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.noSneakCamera.getValue().booleanValue() && entity.isSneaking()) {
            f += 0.08f;
        }
        ...
        FreeLookModule freeLookModule = Advantage.INSTANCE.getModuleManager().getModule(FreeLookModule.class);
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                GlStateManager.rotate(rotationPitch - f22, 1.0f, 0.0f, 0.0f);
                GlStateManager.rotate(rotationYaw - f1, 0.0f, 1.0f, 0.0f);
                GlStateManager.translate(0.0f, 0.0f, (float)(-d3));
                GlStateManager.rotate(f1 - rotationYaw, 0.0f, 1.0f, 0.0f);
                GlStateManager.rotate(f22 - rotationPitch, 1.0f, 0.0f, 0.0f);
            }
        } else {
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        float aspect = Advantage.INSTANCE.getModuleManager().getModule(AspectRatioModule.class).isEnabled() ? ((Double)AspectRatioModule.aspect.getValue()).floatValue() : (float)this.mc.displayWidth / (float)this.mc.displayHeight;
    ...
    private void updateLightmap(float partialTicks) {
        WorldClient world;
        if (this.lightmapUpdateNeeded && (world = this.mc.theWorld) != null) {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            if ((freeLookModule = Advantage.INSTANCE.getModuleManager().getModule(FreeLookModule.class)).isEnabled() && freeLookModule.freeLooked()) {
                float newYaw = freeLookModule.getCameraYaw() + f2 * 0.15f;
                float newPitch = freeLookModule.getCameraPitch() - f3 * 0.15f * (float)i;
                if (newPitch > 90.0f) {
                    newPitch = 90.0f;
                }
                if (newPitch < -90.0f) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Mouse location", new Callable<String>(){
                        final /* synthetic */ EntityRenderer this$0;
                        {
                            this.this$0 = this$0;
                        }
    ...
    private boolean isDrawBlockOutline() {
        ...
        if (flag && !((EntityPlayer)entity).capabilities.allowEdit) {
            ...
            if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                IBlockState iblockstate = this.mc.theWorld.getBlockState(blockpos);
                Block block = iblockstate.getBlock();
                flag = this.mc.playerController.getCurrentGameType() == WorldSettings.GameType.SPECTATOR ? iblockstate.getBlock().hasTileEntity() && this.mc.theWorld.getTileEntity(blockpos) instanceof IInventory : itemstack != null && (itemstack.canDestroy(block) || itemstack.canPlaceOn(block));
            }
        }
        return flag;
    }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        GlStateManager.enableBlend();
        Render3DEvent eventRender3D = new Render3DEvent();
        Advantage.INSTANCE.getEventBus().post(eventRender3D);
        GlStateManager.disableBlend();
        Objects.requireNonNull(this);
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        GL11.glFogfv(2918, this.setFogColorBuffer(this.fogColorRed, this.fogColorGreen, this.fogColorBlue, 1.0f));
        GL11.glNormal3f(0.0f, -1.0f, 0.0f);
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
        float f = -1.0f;
        if (f >= 0.0f) {
            ...
            if (Advantage.INSTANCE.getModuleManager().getModule(CameraModule.class).isEnabled() && CameraModule.noBlindness.getValue().booleanValue()) {
                return;
            }
            float f4 = 5.0f;
            int i = ((EntityLivingBase)entity).getActivePotionEffect(Potion.blindness).getDuration();

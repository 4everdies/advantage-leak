        ...
        for (ModelResourceLocation modelresourcelocation : p_177591_1_) {
            try {
                ...
                try {
                    ...
                    LOGGER.warn("Unable to load variant: " + modelresourcelocation.getVariant() + " from " + String.valueOf(modelresourcelocation), (Throwable)exception);
                }
            }
            catch (Exception exception1) {
                LOGGER.warn("Unable to load definition " + String.valueOf(modelresourcelocation), (Throwable)exception1);
            }
        }
    }
            ...
            try {
                for (IResource iresource : this.resourceManager.getAllResources(resourcelocation)) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("Encountered an exception when loading model definition of '" + String.valueOf(p_177586_1_) + "' from: '" + String.valueOf(iresource.getResourceLocation()) + "' in resourcepack: '" + iresource.getResourcePackName() + "'", exception);
                    }
                    finally {
                ...
                throw new RuntimeException("Encountered an exception when loading model definition of model " + String.valueOf(resourcelocation), ioexception);
            }
        ...
        for (ModelResourceLocation modelresourcelocation : this.variants.keySet()) {
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
                ...
                try {
                    ...
                    LOGGER.warn("Unable to load block model: '" + String.valueOf(resourcelocation) + "' for variant: '" + String.valueOf(modelresourcelocation) + "'", (Throwable)exception);
                }
            }
        }
    }
        ...
        for (Item item : Item.itemRegistry) {
            for (String s : this.getVariantNames(item)) {
                ...
                try {
                    ...
                    LOGGER.warn("Unable to load item model: '" + String.valueOf(resourcelocation) + "' for item: '" + String.valueOf(Item.itemRegistry.getNameForObject(item)) + "'", (Throwable)exception);
                }
            }
        }
    }
    ...
    private void bakeBlockModels() {
        for (ModelResourceLocation modelResourceLocation : this.variants.keySet()) {
            ...
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelResourceLocation).getVariants()) {
                ...
                LOGGER.warn("Missing model for: " + String.valueOf(modelResourceLocation));
            }
            if (i == 0) {
                LOGGER.warn("No weighted models for: " + String.valueOf(modelResourceLocation));
                continue;
            }
            if (i == 1) {
        ...
        for (Map.Entry entry : this.itemLocations.entrySet()) {
            ...
            LOGGER.warn("Missing model for: " + String.valueOf(resourcelocation));
        }
    }
    ...
    private void loadModels() {
        ...
        while (!deque.isEmpty()) {
            ...
            try {
                ...
                LOGGER.warn("In parent chain: " + JOINER.join(this.getParentPath(resourcelocation2)) + "; unable to load model: '" + String.valueOf(resourcelocation2) + "'");
            }
        ...
        IIconCreator iiconcreator = new IIconCreator(){
            ...
            public void registerSprites(TextureMap iconRegistry) {
                for (ResourceLocation resourcelocation : set) {
                    ...
                    this.this$0.sprites.put(resourcelocation, textureatlassprite);
                }
            }
        };

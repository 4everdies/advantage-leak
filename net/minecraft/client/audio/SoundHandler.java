...
ITickable {
    ...
    private void loadSoundResource(ResourceLocation location, SoundList sounds) {
        ...
        block10: for (final SoundList.SoundEntry soundlist$soundentry : sounds.getSoundList()) {
            ...
            soundeventaccessorcomposite.addSoundToEventPool(switch (soundlist$soundentry.getSoundEntryType()) {
                case SoundList.SoundEntry.Type.FILE -> {
                    ...
                    try {
                        ...
                        logger.warn("Could not load sound file " + String.valueOf(resourcelocation1) + ", cannot add it to event " + String.valueOf(location), (Throwable)ioexception);
                        {
                    ...
                    IOUtils.closeQuietly(inputstream);
                    yield new SoundEventAccessor(new SoundPoolEntry(resourcelocation1, soundlist$soundentry.getSoundEntryPitch(), soundlist$soundentry.getSoundEntryVolume(), soundlist$soundentry.isStreaming()), soundlist$soundentry.getSoundEntryWeight());
                }
                case SoundList.SoundEntry.Type.SOUND_EVENT -> new ISoundEventAccessor<SoundPoolEntry>(){
                    ...
                    public int getWeight() {
                        SoundEventAccessorComposite soundeventaccessorcomposite1 = (SoundEventAccessorComposite)this.this$0.sndRegistry.getObject(this.field_148726_a);

...
extends AbstractClientPlayer {
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            ...
            boolean bl = player = this == Minecraft.getMinecraft().thePlayer;
            if (player) {
                Advantage.INSTANCE.getEventBus().post(update);
            }
            super.onUpdate();
            if (this.isRiding()) {
    ...
    public void onUpdateWalkingPlayer() {
        ...
        SprintEvent sprintEvent = new SprintEvent(this.isSprinting());
        Advantage.INSTANCE.getEventBus().post(sprintEvent);
        boolean clientSprintState = sprintEvent.isSprinting();
        ...
        if ((clientSneakState = this.isSneaking()) != this.serverSneakState) {
            if (clientSneakState) {
        ...
        if (this.isCurrentViewEntity()) {
            MotionEvent motionEvent = new MotionEvent(this.lastReportedYaw, this.lastReportedPitch, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, this.rotationYaw, this.rotationPitch, this.onGround);
            Advantage.INSTANCE.getEventBus().post(motionEvent);
            double eventX = motionEvent.getPosX();
            double eventY = motionEvent.getPosY();
            double eventZ = motionEvent.getPosZ();
            float eventYaw = motionEvent.getYaw();
            float eventPitch = motionEvent.getPitch();
            boolean eventOnGround = motionEvent.isOnGround();
    ...
    public void sendChatMessage(String message) {
        if (Advantage.INSTANCE.getCommandHandler().execute(message) || message.startsWith(CommandHandler.CHAT_PREFIX)) {
            return;
        }
    ...
    public void onLivingUpdate() {
        ...
        ItemSlowdownEvent slowDownEvent = new ItemSlowdownEvent(0.2f, 0.2f, this.isUsingItem());
        Advantage.INSTANCE.getEventBus().post(slowDownEvent);
    ...
    public void moveEntity(double x, double y, double z) {
        MovePlayerEvent moveEvent = new MovePlayerEvent(x, y, z);
        Advantage.INSTANCE.getEventBus().post(moveEvent);
